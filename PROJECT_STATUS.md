...
- **140 tests passing**, all five packages typecheck clean under strict TS.
...
TwoColumn / nested editing (ADR-021) and `imdx dev` watch mode (ADR-026) have
landed. Remaining:

1. **Media library UI** — the `POST /media` endpoint exists; needs a browser.
2. TwoColumn polish — nested drop indicators and per-region slash menu.
3. A GitHub-mode deploy guide (drop `localMode`; wire OAuth + `GitHubProvider`).
4. HMR: push registry changes from `imdx dev` into a running editor.
