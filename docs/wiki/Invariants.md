...
| 2 | **Props are JSON.** Only literals/arrays/plain-objects/unary-minus. Enforced on both the content side and the defineIMDX-config side; they change together. | `core/props.ts`, `cli/static-eval.ts` | 002 |
...
| 4 | **Diagnostic codes are stable API.** IMDX001–009 meanings are fixed (008/009 = frontmatter). Add codes; never repurpose. | `core/types.ts`, `core/validate.ts`, `core/frontmatter.ts` | 005, 025 |
...
- **Build order.** Dependents resolve `@imdx/core` through its built `dist/`.
  Root `pnpm test`/`build`/`check` build core first, so a clean
  `pnpm install && pnpm test` works. Running a single dependent's vitest
  directly after editing core requires `pnpm --filter @imdx/core build` first.
