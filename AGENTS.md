...
| `@imdx/editor` | `packages/editor` | flat React UI done | `buildSchema(registry)` (registry → ProseMirror schema), `fromMdast`/`toMdast` converters, mark canonicalization, `imdx_raw` fallback, `printPropValue`. Flat React editor under `@imdx/editor/react` — React NodeViews on **raw ProseMirror** (ADR-023, supersedes ADR-019's TipTap), rail/slash menu/prop panel/live-source pane. Main entry stays React-free. Nested editing (TwoColumn) pending. |
