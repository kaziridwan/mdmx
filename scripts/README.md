...
| `generate.sh` | `pnpm generate` | Regenerate `.imdx/registry.*` for every example app. |
| `test.sh` | `pnpm test` | Run all package test suites. |
| `check.sh` | `pnpm verify` | Typecheck everything, then run all tests (pre-push gate). |
| `clean.sh` | `pnpm clean` | Remove build artifacts (`dist/`, `.next/`, generated `.imdx/`, tsbuildinfo). `--all` also wipes `node_modules`. |
