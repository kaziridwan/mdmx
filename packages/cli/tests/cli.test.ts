...
afterAll(() => {
  rmSync(join(APP, ".imdx"), { recursive: true, force: true });
});

describe("imdx generate", () => {
  it("loads config from imdx.config.json", () => {
    expect(config.components).toBe("components/imdx/**/*.tsx");
    expect(config.outDir).toBe(".imdx");
  });
  ...
  it("merges defineIMDX config over inference", () => {
    const callout = result.spec.components.find((c) => c.name === "Callout")!;
  ...
  it("emits a loadable registry.json with a hash", () => {
    const raw = JSON.parse(
      readFileSync(join(APP, ".imdx/registry.json"), "utf8"),
    ) as RegistrySpec;
  ...
  it("emits a binding module with correct imports", () => {
    const ts = readFileSync(join(APP, ".imdx/registry.ts"), "utf8");
    expect(ts).toContain('import Callout from "../components/imdx/Callout";');
    expect(ts).toContain('import { Chart } from "../components/imdx/Chart";');
...
describe("imdx check", () => {
  it("passes the valid file and flags the invalid one", async () => {
    expect(existsSync(join(APP, ".imdx/registry.json"))).toBe(true);
    ...
    expect(codes).toContain("IMDX001"); // <Mystery />
    expect(codes).toContain("IMDX002"); // series={getSeries()}
    expect(codes).toContain("IMDX004"); // heading inside rich-text
...
describe("collections", () => {
  it("normalizes config collections into the emitted registry spec", async () => {
    const withCollections: IMDXConfig = {
      ...config,
      collections: {
        posts: {
          dir: "content/posts",
