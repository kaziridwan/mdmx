...
import type { StudioClient, StudioHost } from "./client.js";
 ...
 * the preview.
 */
export function StudioEditorView({ name,
  client,
  host,
}: { name?: string;
  client: StudioClient;
  ...
  const entry = editing ? client.entries.find((e) => e.def.name === name) : undefined;
  ...
  const validation = useMemo(() => {
    ...
    const taken = new Set(
      client.entries.map((e) => e.def.name).filter((n) => n !== (editing ? name : undefined)),
    );
    return validateStudioComponent(candidate, taken);
  }, [candidate, client.entries, editing, name]);
  ...
  const onSave = async () => {
    ...
    try {
      ...
      host.navigate(host.hrefFor("studio"));
    } catch (err) {
    ...
    return (
      <div className="mdmx-dash-view">
        <div className="mdmx-dash-empty">
          ...
          <host.Link className="mdmx-dash-button" href={host.hrefFor("studio")}>
            Back to studio
          </host.Link>
        </div>
      </div>
  ...
  return (
    <div className="mdmx-dash-view mdmx-studio-editor">
      <header className="mdmx-dash-view-head">
        <div>
          <host.Link className="mdmx-toolbar-back" href={host.hrefFor("studio")}>
            ← Studio
          </host.Link>
          ...
          {editing ? (
            <button
              ...
              onClick={() => {
                ...
                client
                  .eject(name!)
                  .then((r: { path: string; note: string }) =>
                    setEjectNote(`Wrote ${r.path}. ${r.note}`),
                  )
